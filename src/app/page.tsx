...
function IntakeStep({ values, onChange, onNext }: {
  values: Intake; onChange: (v: Intake) => void; onNext: () => void;
}) {
  ...
  return (
    <div style={{ padding: "24px 20px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <p style={{ color: "#666", fontSize: 13, margin: "0 0 24px", lineHeight: 1.7 }}>Measures your Kingdom Readiness Score across eight spiritual domains. Be honest — this is a mirror, not a test.</p>
      {([
        { key: "name" as const, label: "Full Name *", type: "text", placeholder: "Your full name" },
        { key: "email" as const, label: "Email (optional)", type: "email", placeholder: "you@email.com" },
        { key: "phone" as const, label: "Phone (optional)", type: "tel", placeholder: "(555) 000-0000" },
      ]).map(f => (
        <div key={f.key} style={{ marginBottom: 14 }}>
          <label style={{ color: "#aaa", fontSize: 12, fontWeight: 600, display: "block", marginBottom: 5 }}>{f.label}</label>
          <input type={f.type} placeholder={f.placeholder} value={values[f.key]} onChange={e => onChange({ ...values, [f.key]: e.target.value })}
...
function S1Step({ question, selected, onSelect, onNext, index, total }: {
  ...
  onNext: () => void; index: number; total: number;
}) {
  return (
    <div style={{ padding: "20px", maxWidth: 480, margin: "0 auto" }}>
      <p style={{ color: "#555", fontSize: 12, fontWeight: 600, margin: "0 0 16px" }}>Section 1 · {index + 1} of {total}</p>
      ...
      {question.options.map(opt => (
        <button key={opt.label} onClick={() => onSelect(opt.label)}
          style={{ display: "block", width: "100%", textAlign: "left", padding: "14px 18px", marginBottom: 10, background: selected === opt.label ? "#1a0000" : "#111", border: `2px solid ${selected === opt.label ? "#cc0000" : "#1e1e1e"}`, borderRadius: 12, color: selected === opt.label ? "#fff" : "#999", fontSize: 13, cursor: "pointer", lineHeight: 1.5 }}>
          <span style={{ color: "#cc0000", fontWeight: 700, marginRight: 8 }}>{opt.label}</span>{opt.text}
        </button>
      ))}
      <button onClick={onNext} disabled={!selected}
        style={{ width: "100%", padding: 14, marginTop: 6, background: selected ? "#cc0000" : "#1a1a1a", color: selected ? "#fff" : "#444", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: selected ? "pointer" : "not-allowed" }}>
...
function S2Step({ answers, onChange, onNext }: {
  answers: S2Answers; onChange: (a: S2Answers) => void; onNext: () => void;
}) {
  ...
  return (
    <div style={{ padding: "20px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <p style={{ color: "#666", fontSize: 12, margin: "0 0 16px", lineHeight: 1.6 }}>Rank all four words per row: <strong style={{ color: "#aaa" }}>4 = Most Like Me</strong> → <strong style={{ color: "#aaa" }}>1 = Least</strong>. Each number once per row.</p>
          ...
          <div key={row.rowNumber} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 10, padding: "10px 6px", background: "#111", borderRadius: 10, border: "1px solid #1e1e1e" }}>
            {(["lion", "otter", "gr", "beaver"] as const).map((col, ci) => (
              <div key={col} style={{ textAlign: "center" }}>
                <p style={{ color: "#ccc", fontSize: 10, margin: "0 0 5px", lineHeight: 1.3 }}>{[row.lion, row.otter, row.gr, row.beaver][ci]}</p>
                <select value={a[col] || ""} onChange={e => setRank(row.rowNumber, col, Number(e.target.value))}
    ...
    <div style={{ padding: "20px", maxWidth: 480, margin: "0 auto" }}>
      ...
      {(["A", "B"] as const).map(opt => (
        <button key={opt} onClick={() => onSelect(opt)}
          style={{ display: "block", width: "100%", textAlign: "left", padding: "20px 18px", marginBottom: 14, background: selected === opt ? "#1a0000" : "#111", border: `2px solid ${selected === opt ? "#cc0000" : "#1e1e1e"}`, borderRadius: 14, color: selected === opt ? "#fff" : "#999", fontSize: 14, cursor: "pointer", lineHeight: 1.6 }}>
          {opt === "A" ? question.optionA : question.optionB}
        </button>
      ))}
      <button onClick={onNext} disabled={!selected}
        style={{ width: "100%", padding: 14, marginTop: 4, background: selected ? "#cc0000" : "#1a1a1a", color: selected ? "#fff" : "#444", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: selected ? "pointer" : "not-allowed" }}>
...
function AssessmentFlow({ telegramUserId, telegramName, onComplete }: {
  telegramUserId: number; telegramName: string; onComplete: () => void;
}) {
  const [step, setStep] = useState<AssessmentStep>("intake");
  const [intake, setIntake] = useState<Intake>({ name: telegramName, email: "", phone: "" });
  ...
  async function submit() {
    ...
    try {
      ...
      if (data.error || !data.leaderId) throw new Error(data.error ?? "Submission failed");
      onComplete();
    } catch (e) {
  ...
  function startAssessment() {
    fetch("/api/assessment/intake", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...intake, telegram: telegramName }) }).catch(() => {});
    setStep("s1");
  }
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      {error && <div style={{ background: "#1a0000", border: "1px solid #cc0000", borderRadius: 8, margin: "12px 20px", padding: "10px 14px", color: "#ff6666", fontSize: 13 }}>{error}</div>}
      {step === "intake" && <IntakeStep values={intake} onChange={setIntake} onNext={startAssessment} />}
      {step === "s1" && <S1Step question={S1_QUESTIONS[q1Index]} selected={s1Answers[S1_QUESTIONS[q1Index].number]} onSelect={ans => setS1Answers(p => ({ ...p, [S1_QUESTIONS[q1Index].number]: ans }))} onNext={() => q1Index < S1_QUESTIONS.length - 1 ? setQ1Index(i => i + 1) : setStep("s2")} index={q1Index} total={S1_QUESTIONS.length} />}
      {step === "s2" && <S2Step answers={s2Answers} onChange={setS2Answers} onNext={() => setStep("s3")} />}
      {step === "s3" && <S3Step question={S3_QUESTIONS[q3Index]} selected={s3Answers[S3_QUESTIONS[q3Index].number]} onSelect={ans => setS3Answers(p => ({ ...p, [S3_QUESTIONS[q3Index].number]: ans }))} onNext={() => q3Index < S3_QUESTIONS.length - 1 ? setQ3Index(i => i + 1) : submit()} index={q3Index} total={S3_QUESTIONS.length} />}
      {step === "submitting" && (
...
export default function Page() {
  ...
  if (!profile) return <AssessmentFlow telegramUserId={userId} telegramName={userName} onComplete={() => fetchProfile(userId!)} />;
